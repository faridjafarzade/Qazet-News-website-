...
import beans.GeneralSessionBean;
 ...
 * @author Farid
 */
@WebFilter(filterName = "adminController", urlPatterns = {"/ap/*"}, dispatcherTypes = {DispatcherType.REQUEST})
public class adminController implements Filter {
    ...
    private void doBeforeProcessing(RequestWrapper request, ResponseWrapper response)
        ...
        if (debug) {
            log("adminController:DoBeforeProcessing");
        }
    ...
    private void doAfterProcessing(RequestWrapper request, ResponseWrapper response)
            throws IOException, ServletException {
        if (debug) {
            log("adminController:DoAfterProcessing");
        }
...
//    
        String password = (String) request.getSession().getAttribute("v1");
        String editor=(String) request.getSession().getAttribute("v2");
        if (request.getRequestURL().indexOf("faces") == -1 || !request.getRequestURI().endsWith("/faces/ap/login.xhtml") && password == null) {
            response.sendRedirect(request.getContextPath() + "/faces/ap/login.xhtml?faces-redirect=true");
            
        } else if (password != null && (request.getRequestURI().endsWith("/") || request.getRequestURI().endsWith("/faces/ap/login.xhtml") || request.getRequestURI().endsWith("/login.xhtml") || request.getRequestURL().indexOf("faces") == -1)) {
            response.sendRedirect(request.getContextPath() + "/faces/ap/index.xhtml");
    ...
    public void init(FilterConfig filterConfig) {
        ...
        if (filterConfig != null) {
            if (debug) {
                log("adminController: Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        ...
        StringBuffer sb = new StringBuffer("adminController(");
    ...
    class RequestWrapper extends HttpServletRequestWrapper {
        ...
        public void setParameter(String name, String[] values) {
            if (debug) {
                System.out.println("adminController::setParameter(" + name + "=" + values + ")" + " localParams = " + localParams);
            }
        ...
        public String getParameter(String name) {
            if (debug) {
                System.out.println("adminController::getParameter(" + name + ") localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public String[] getParameterValues(String name) {
            if (debug) {
                System.out.println("adminController::getParameterValues(" + name + ") localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public Enumeration getParameterNames() {
            if (debug) {
                System.out.println("adminController::getParameterNames() localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public Map getParameterMap() {
            if (debug) {
                System.out.println("adminController::getParameterMap() localParams = " + localParams);
            }
            if (localParams == null) {
